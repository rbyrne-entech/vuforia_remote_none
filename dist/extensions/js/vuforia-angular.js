                ...
                dataset.trackers.forEach(function(tracker) {
                    ...
                    var successCallback = function() {
                        ...
                        tracker.assets.forEach(function(asset) {
                            VF_ANG.processAsset(tracker, asset, cssApplyFunction);
                        });
                    ...
                    asset.addAsset = function() {
                        ...
                        VF_ANG.addAsset(renderer, asset, function() {
                            ...
                            renderer.add3DImage(
                                tracker.name,
                                element.attr("id"),
                                VF_ANG.textToImage(element, properties),
                                undefined,
                                undefined,
                                undefined,
                                undefined,
                            ...
                            ArSensor.loadImage(image, element[0], function(imageObj) {
                                ...
                                VF_ANG.addAsset(renderer, asset, function() {
                                    ...
                                    renderer.add3DImage(tracker.name,
                                                        ...
                                                        canvasData.imagePhysicalWidth,
                                                        canvasData.imagePhysicalHeight,
                                                        element.attr("pivot"),
                                                        addSuccessCallback,
                                                        addFailureCallback);
                                });
...
VF_ANG.createAddSetTriggerFunctions = function(addFunction, setFunction) {
    ...
    var watchFunction = function watchFunction (newValues, oldValues) {
        switch (currentStage) {
            ...
            case VF_ANG.WATCH_STAGE_ADD_IN_PROGRESS: {
                ...
                // a change between those two but this would complicate things even further.
...
VF_ANG.setupWatchWithAddSetConvention = function setupWatchWithAddSetConvention (scope, element, attributeNames, addFunction, setFunction) {
    var addSetTriggerFunctions = VF_ANG.createAddSetTriggerFunctions(addFunction, setFunction);
...
VF_ANG.applyWatch = function(scope,element,renderer,id) {
    ...
    scope.$watch(function () { return nodeId + element.attr('billboard') + element.attr('occlude') + element.attr('hidden') + element.attr('phantom') + element.attr('opacity') + element.attr('decal') + element.attr('shader') + element.attr('force-hidden') + element.attr('experimental-one-sided'); }, function (value) {
