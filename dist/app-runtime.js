...
(function () {
    ...
    if (history.state && history.state.isFirstPage) {
        ...
        if (typeof(history.replaceState) === "function") {  // should be present in all our browsers, but not necessarily tests
            if (history.state) {
    ...
    function backButtonHandler(e, $ionicHistory, ionicPlatform) {
        ...
        } else if (!isFirstPage && vuforia && typeof(vuforia.goBack) === "function") {  // vuforia from vuforia.js
    ...
    twx.app.isPreview = function () {
        if ('URL' in window && typeof window.URL === 'function') {
            ...
            return (getParameterByName('preview', window.parent ? window.parent.location.href : window.location.href) === 'true');
        }
    };
    ...
    twx.app.getAppEventsWithHandlers = function () {
        ...
        _.each(document.querySelectorAll('twx-app-event'), function(appEventEl) {
            ...
            if(appEvent.attr('expression') || document.querySelector('twx-eventbind[source-event="' + appEvent.attr('name') + '"][source-type="event"]')) {
                appEvents.push( {name: appEvent.attr('name'), voiceAlias: appEvent.attr('voicealias')} );
            }
        });
        return appEvents.sort(function(a, b) { return (a.name < b.name) ? -1 : (a.name > b.name) ? 1 : 0; });
    };
    ...
    app.directive('twxWidget', function(){
        return {
            ...
            controller: function ($scope, $element, $attrs, $rootScope, $timeout, $filter, DataConnector, filterService) {
                ...
                this.addService = function(name, elem){
                    ...
                    var serviceHandler = function (evt, data) {
                      if (delegate[name]) {
                        ...
                        var svcHandler = $element[0].querySelector('[twx-service-handler~="'+name+'"]') ||
                ...
                if ($element[0].querySelector('[twx-native-events]')) {
                    ...
                    var attachListener = function(name) {
                        if(!alreadyAttachedListener[name]) {
                            ...
                            var el = $element[0].querySelector('[twx-native-events~="'+name+'"]') ||
                ...
                this.addEventBinding = function(attrs, eventBindingElem) {
                    ...
                    function addEventData(eventBindingElem, currentData) {
                        ...
                        _.each(eventBindingElem.find('twx-event-data'), function(dataEl) {
                            ...
                            data[dataEl.attr('data-name') || dataEl.attr('name')] = twx.app.convertStringPropertyToBasetype((dataEl.attr('data-value') || dataEl.attr('value')), dataEl.attr('base-type'));
                        });
                        return data;
                    }
                    ...
                    if (eventSourceType === 'event') {
                        ...
                        angular.element(eventBindingElem).on('twxEventEmitted', function(event) {
                            $rootScope.$broadcast(handler, addEventData(eventBindingElem, event.detail));
                        });
                    } else {
                        ...
                        $scope.$on(eventName, function(event, data) {
                            $rootScope.$broadcast(handler, addEventData(eventBindingElem, data));
                        });
                    }
                };
            }
   ...
   */
    ...
    twx.app.fn._setStyleCount = function(n) { styleCount = n;};
        ...
        if (loc && el) {
            ...
            if (loc.hash) {
                ...
                if (loc.hash.indexOf('?') > -1) {
                  params = new URLSearchParams(loc.hash.substring(loc.hash.indexOf('?')));
                  params.delete('vumark');
                  params.delete('thing');
                  params.delete('template');
                }
                else {
                  //TM=none urls have no params, but need the view removed
                  params = new URLSearchParams();
                }
                if (offlineURL.indexOf('#/') > 0) {
                  //In case of bookmarks, remove default view name and hash params
                  offlineURL = offlineURL.substring(0, offlineURL.indexOf('#/') + 2);
                }
                else {
    ...
    */
        ...
        if (params.has('expId') && !hashViewRE.test(loc.hash)) {
          ...
          if (startExp) {
            ...
            if (originalHash) {
              ...
              var orig = originalHash.indexOf('?') > -1 ? originalHash.substring(originalHash.indexOf('?'), originalHash.length) : '';
              newHash += orig;
            }
            loc.hash = newHash;
          }
     ...
     * @return true if any of the tracked objects match a target within the experience,
